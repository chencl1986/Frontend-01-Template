...
function getCharPoint(char) {
    var charPoint = char.codePointAt(0);
...
function convertStringToNumber(string, radix) {
    ...
    var chars = newString.split('');
    var zeroCodePoint = '0'.codePointAt(0); // 0的码点
    var maxCodePoint = zeroCodePoint + radix - 1; // 当前进制最大值
    var result = 0; // 最终结果
    var number = 0; // 整数转换结果
    var fraction = 1; // 小数转换结果
    var i = 0; // 用于处理的索引
    // 将整数和小鼠分段处理，此处处理整数部分
    while (i < chars.length && chars[i] !== '.') {
        var charPoint = getCharPoint(chars[i]);
        // 如果超出进制最大值范围，直接返回NaN
